...
for (i in 1:dim(rejected_null)[1]) {
  for (j in 1:dim(rejected_null)[2]) {
    for (k in 1:dim(rejected_null)[3]) {
      if(gamma[j] == 0){
        p_1 <- p_0 + 0.01 * coef # alternative is true
      } else {
...
```{r, results = "asis", echo = F}
rejected_null_calc_min_n[,,2] %>% as.data.frame() %>% 
  pandoc.table(split.tables = Inf, caption = "sides = 2")
```
...
CI <- function(p_1_hat, n_1, p_0_hat, n_0, alpha, h) {
  ...
  ans <- list(lower = point_estimate - rhs, 
...
CI_coverage <- array(
  ...
  dimnames = list(paste0("#tests = ", h), paste0("alpha = ", alpha), 
                  paste0("diff = ", diff))
)
    ...
    for(k in 1:dim(CI_coverage)[3]){
      ...
      CI_mat <- mapply(
        function(p_1_hat, p_0_hat) {
          ...
          in_ci <- ci$lower < diff[k] & diff[k] < ci$upper 
          return(in_ci)
        },
        split(p_1_hat, rep(1:ncol(p_1_hat),
                           each = nrow(p_1_hat)
...
```{r, results = "asis", echo = F}
CI_coverage[,,1] %>% as.data.frame() %>% 
  pandoc.table(split.tables = Inf, caption = paste0("diff = ", diff[1]))
```

```{r, results = "asis", echo = F}
CI_coverage[,,2] %>% as.data.frame() %>% 
  pandoc.table(split.tables = Inf, caption = paste0("diff = ", diff[2]))
```

```{r, results = "asis", echo = F}
CI_coverage[,,3] %>% as.data.frame() %>% 
  pandoc.table(split.tables = Inf, caption = paste0("diff = ", diff[3]))
```
