...
To that end we'll construct a test such that we reject the null if the difference $\hat{p_1} - \hat{p_0}$ exceeds some threshold $C$ with probability $\alpha$ when the null is in fact true:
...
```{r, results = "asis", echo = F}
rejected_null[,4,2] %>% as.data.frame() %>% pandoc.table(split.tables = Inf, caption = "sides = 2")
```
